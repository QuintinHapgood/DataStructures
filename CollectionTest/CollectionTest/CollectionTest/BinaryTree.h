...
template<typename T>
bool BinaryTree<T>::Add(const T &value)
{
	Node *del = new Node();
	del->value = value;
...
template<typename T>
bool BinaryTree<T>::Add(Node *del)
{
	if (root == nullptr)
	{
		root = del;
...
template<typename T>
bool BinaryTree<T>::Remove(Node *parent, Node *newNode)
{
...
template<typename T>
bool BinaryTree<T>::ContainsValue(const T &value)
{
...
template<typename T>
bool BinaryTree<T>::Add(Node *parent, Node *newNode)
{
	if (newNode->value < parent->value)
	{
...
template<typename T>
void BinaryTree<T>::RecursiveDFS(Node * parent, std::function<bool(const T&)> fn)
{
	if (parent == nullptr)
		return;
...
template<typename T>
void BinaryTree<T>::ForEach(std::function<bool(const T&)> fn)
{
